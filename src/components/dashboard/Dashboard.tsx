    ...
    <div className="space-y-6">
      ...
      <div className="relative text-center py-6">
        ...
        <div className="pointer-events-none absolute inset-0 -z-10 [mask-image:radial-gradient(ellipse_at_center,black,transparent_65%)]">
          <motion.div className="absolute left-1/4 top-4 w-24 h-24 rounded-full bg-pink-200/60 blur-2xl" animate={{ y: [0, -8, 0] }} transition={{ repeat: Infinity, duration: 6 }} />
          <motion.div className="absolute right-1/4 top-8 w-20 h-20 rounded-full bg-purple-200/60 blur-xl" animate={{ y: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 7 }} />
        </div>
      ...
      {/* Quick Stats Grid */}
      <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-4" initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.08 } } }}>
        {/* Next Period */}
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <Button variant="outline" className="h-20 flex flex-col gap-2" onClick={() => setShowPeriod(true)}>
                <Plus className="w-5 h-5" />
                Log Period
              </Button>
              <Button variant="outline" className="h-20 flex flex-col gap-2" onClick={() => setShowSymptoms(true)}>
                <Activity className="w-5 h-5" />
                Add Symptoms
              </Button>
              <Button variant="outline" className="h-20 flex flex-col gap-2" onClick={() => setShowBreathing(true)}>
                <Heart className="w-5 h-5" />
                Breathing
              </Button>
              <Button variant="outline" className="h-20 flex flex-col gap-2" onClick={() => setShowCalendar(true)}>
                <Calendar className="w-5 h-5" />
                Calendar
              </Button>
            </div>
      ...
      {/* Modals */}
      <Modal open={showPeriod} onClose={() => setShowPeriod(false)} title="Log Period">
        <PeriodLogForm onCancel={() => setShowPeriod(false)} onComplete={() => setShowPeriod(false)} />
      </Modal>
      <Modal open={showSymptoms} onClose={() => setShowSymptoms(false)} title="Log Symptoms">
        <SymptomLogForm onCancel={() => setShowSymptoms(false)} onComplete={() => setShowSymptoms(false)} />
      </Modal>
      <Modal open={showBreathing} onClose={() => setShowBreathing(false)} title="Breathing Exercise">
        <BreathingExercise />
      </Modal>
      <Modal open={showCalendar} onClose={() => setShowCalendar(false)} title="Cycle Calendar">
        <CalendarView />
      </Modal>
